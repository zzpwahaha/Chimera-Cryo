/****************************************************************************************
 *Note: MAY 03 2023, increase ramp array length to accomidate more ramp segments
...
int32_t ramp0[512][6];  //ramp parameters for ch0 (Fs,Fe,R#,Rleng,dF,dt)
int32_t ramp1[512][6];  //ramp parameters for ch1 (Fs,Fe,R#,Rleng,dF,dt)
...
void processData(){
  float para[5];
  uint32_t paraint0[512][5];
  uint32_t paraint1[512][5];
  ...
  //check if there are errors in the data or the serial communication timeout is reached or ramp segments exceed array size of 512
  if (error == false && serialTimeout < 10000 && rampCount0 < 512 && rampCount1 < 512 ){
