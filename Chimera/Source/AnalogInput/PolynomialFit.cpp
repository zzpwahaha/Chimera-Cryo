...
(size_t n, double* datax, double* datay, unsigned order, bool useSqrt,
    ...
    : order(order), useSqrt(useSqrt), numOfPara(order + 2 * useSqrt)
...
double PolynomialFit::polynomial(std::vector<double> para, double t)
{
    ...
    if (useSqrt) {
        res += para[order] * std::sqrt(t - para[order + 1]);
    }
    return res;
}
...
std::vector<double> PolynomialFit::calcFittedGaussian()
{
    ...
    std::vector<double> para(p->data, p->data + p->size * p->stride);
...
{
    return std::vector<double>(p->data, p->data + p->size * p->stride);
}

std::vector<double> PolynomialFit::confidence95Interval() const
{
    return std::vector<double>(confid95->data, confid95->data + confid95->size * confid95->stride);
}
...
int PolynomialFit::func_df(const gsl_vector* p, void* datafit, gsl_matrix* J)
{
    ...
    for (size_t i = 0; i < dataf->n; ++i)
    {
        ...
        if (useSqrt) {
            double b0 = gsl_vector_get(p, order);
            double b1 = gsl_vector_get(p, order + 1);
            gsl_matrix_set(J, i, order, -std::sqrt(tmp - b1));
            gsl_matrix_set(J, i, order + 1, b0 / std::sqrt(tmp - b1) / 2);
        }
    }
    return GSL_SUCCESS;
}
...
int PolynomialFit::func_fvv(const gsl_vector* p, const gsl_vector* v, void* datafit, gsl_vector* fvv)
{
    ...
    for (size_t i = 0; i < dataf->n; ++i)
    {
        ...
        if (useSqrt) {
            ...
            double b0 = gsl_vector_get(p, order);
            double b1 = gsl_vector_get(p, order + 1);
            double vb0 = gsl_vector_get(v, order);
            double vb1 = gsl_vector_get(v, order + 1);
