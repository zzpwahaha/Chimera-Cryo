...
void CruncherThreadWorker::handleImage (){
	...
	while (true) {
		...
		if (image.picStat.picNum % debugPicsPerRep == 0) {
			...
			flog << "From Cruncher thread: get image number" << image.picStat.picNum << " at " << std::chrono::duration_cast<std::chrono::nanoseconds>(input->catchPicTimes->back() - input->imageGrabTimes->back()).count() / 1e6 << "ms, relative to grabber thread" << fendl;
		}
...
void CruncherThreadWorker::handleRearrangement(AtomImage atomImage)
{
	...
	flog << "From Cruncher thread: ready to calculated rearrange for image number" << atomImage.picStat.picNum << " at " << std::chrono::duration_cast<std::chrono::nanoseconds>(chronoClockHR::now() - input->imageGrabTimes->back()).count() / 1e6 << "ms, relative to grabber thread" << fendl;
	...
	flog << "From Cruncher thread: ready to trigger rearrange for image number" << atomImage.picStat.picNum << " at " << std::chrono::duration_cast<std::chrono::nanoseconds>(input->finTimes->back() - input->imageGrabTimes->back()).count() / 1e6 << "ms, relative to grabber thread" << fendl;
}