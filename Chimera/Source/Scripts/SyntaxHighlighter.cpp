...
SyntaxHighlighter::SyntaxHighlighter (ScriptableDevice device, QTextDocument* parent) : 
	...
	if (device == ScriptableDevice::Master) {
		...
		addRules ({ "sin","cos","tan","exp","ln","log10","int","sign","var"}, QColor(42, 161, 152), true, true);
	}
		...
		addRules({ "set", "setmove",  "xoffset", "yoffset", "filterx", "filtery", "paintx", "painty",
		...
		addRules({ "var" }, QColor(66, 135, 245), true, true);
	}
