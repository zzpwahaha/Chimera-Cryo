...
00000000000000000000"""
    ...
    def lifetime(self, exp_idx, exp_name_prefix, exp_name_postfix = '', timeout_control={"use": True, "timeout": 6000}):
        ...
        config_file.config_param.update_scan_dimension(0, new_ranges=[
            ScanRange(index=0,left_inclusive=True, right_inclusive=True, variations=11),
            ScanRange(index=1,left_inclusive=True, right_inclusive=True, variations=4),])
        config_file.config_param.update_variable("time_scan_us", scan_type="Variable", 
                                                new_initial_values=[0.01,29.5], new_final_values=[25.01,97]) # 50, 5x14
...
if __name__ == "__main__":
    ...
    NUM_LIFETIME_SEGMENTS = 1
    LIFETIME_REPETITIONS = 60*NUM_LIFETIME_SEGMENTS
    RABI_420_FREQ = 587. #583.8 #582.795 #587.9 #585.037 #583.868 #529.08 #578.048   #578.091 #577.977 #578.096 #576.334 #578.271 #578.656 #578.56 # MHz
    AVALANCHE_420_FREQ = RABI_420_FREQ-1 # MHz

    CURRENT_EOM_FREQ = 586.  #RABI_420_FREQ
    ...
    # INJECTED_FREQS = [3.25,3,2.5,2.25,2,1.5,1] #mV
    INJECTED_FREQS = [2,1.5,1] #mV

    sdg2042 = SDG2042X_NOISEmode.SDG2042XNoise(resource=SDG2042X_IP, channel=1, output_load="HZ")
    sdg2042.query_settings()
    sdg2042.enable_sine(2e6,0.01)

    for idn, injected_freq in enumerate(INJECTED_FREQS):
        EXP_NAME_PREFIX = EXP_NAME_PREFIX_BASE + f'-FREQ{injected_freq:.2f}MHz-Attempt3'
