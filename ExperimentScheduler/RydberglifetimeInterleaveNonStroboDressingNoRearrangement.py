...
00000000000000000000"""
    ...
    def lifetime(self, exp_idx, exp_name_prefix, exp_name_postfix = '', timeout_control={"use": True, "timeout": 6000}):
        script_name = "rydberg_420_1013_excitation_SLM_dressing_rearrangement_nostrobing_NoRearrangement.mScript"
        ...
        config_file.config_param.update_scan_dimension(0, new_ranges=[
            ScanRange(index=0,left_inclusive=True, right_inclusive=True, variations=4),
            ])

        config_file.config_param.update_variable("time_scan_us", scan_type="Variable", 
                                                new_initial_values=[25], new_final_values=[40]) # 50, 5x14
...
if __name__ == "__main__":
    ...
    EXP_NAME_PREFIX = "N-50-0.01-1x1-0.0V-NORNG2"

    NUM_LIFETIME_SEGMENTS = 32
    LIFETIME_REPETITIONS = 600*NUM_LIFETIME_SEGMENTS
    RABI_420_FREQ = 583.439 #577.62 #583.524 #577.624 #587. #583.8 #582.795 #587.9 #585.037 #583.868 #529.08 #578.048   #578.091 #577.977 #578.096 #576.334 #578.271 #578.656 #578.56 # MHz
    AVALANCHE_420_FREQ = RABI_420_FREQ-1 # MHz

    CURRENT_EOM_FREQ = 583.439  #RABI_420_FREQ
