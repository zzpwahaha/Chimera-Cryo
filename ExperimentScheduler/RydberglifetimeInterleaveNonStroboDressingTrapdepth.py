...
analysis_locs = da.DataAnalysis(year='2026', month='May', day='23', data_name='data_1', n_cluster_row=5,
...
00000000000000000000"""
    ...
    def lifetime(self, trap_depth, exp_idx, exp_name_prefix, exp_name_postfix = '', timeout_control={"use": True, "timeout": 6000}):
        script_name = "rydberg_420_1013_excitation_SLM_dressing_rearrangement_nostrobing_trapdepth.mScript"
        ...
        config_file.config_param.update_variable("fraction_scan", constant_value=trap_depth)
        config_file.config_param.update_variable("ryd420_amplitude", constant_value=-0.0096) #-0.0093 #-0.0069 # -0.0072 #-0.0096
        config_file.config_param.update_variable("ryd1013_amplitude", constant_value=4) #4, 2.25
    ...
    def rabi(self, trap_depth, exp_idx, exp_name_prefix, exp_name_postfix = '', timeout_control={"use": True, "timeout": 1800}):
        script_name = "rydberg_420_1013_excitation_SLM_dressing_rearrangement_nostrobing_Rabi_trapdepth.mScript"
    ...
    def resonance(self, trap_depth, exp_idx, exp_name_prefix, exp_name_postfix = '', timeout_control={"use": True, "timeout": 1800}):
        script_name = "rydberg_420_1013_excitation_SLM_dressing_rearrangement_nostrobing_Rabi_trapdepth.mScript"
        gscript_name = "C:/Chimera/Chimera-Cryo/Configurations/CryoTweezerLoading/rearrangement_1x7_Rabi.gScript"
...
if __name__ == "__main__":
    ...
    EXP_NAME_PREFIX_BASE = "N-60-0.01-5x14-0.0V"

    NUM_LIFETIME_SEGMENTS = 2
    LIFETIME_REPETITIONS = 100*NUM_LIFETIME_SEGMENTS
    RABI_420_FREQ = 577.600 #587. #583.8 #582.795 #587.9 #585.037 #583.868 #529.08 #578.048   #578.091 #577.977 #578.096 #576.334 #578.271 #578.656 #578.56 # MHz
    AVALANCHE_420_FREQ = RABI_420_FREQ-1 # MHz

    CURRENT_EOM_FREQ = 577.60 #576.630  #RABI_420_FREQ
    EOM_FREQ_AT_0p01_DEPTH = 577.600
    ...
    TRAPDEPTH = [0.1, 0.03, 0.05, 0.15, 0.2, 0.01]
    def get_rough_resonance(trapdepth):
        return EOM_FREQ_AT_0p01_DEPTH + (trapdepth-0.01)/0.09*1.2

    for trapdepth in TRAPDEPTH:
        EXP_NAME_PREFIX = EXP_NAME_PREFIX_BASE + f'-TD{trapdepth:.2f}'
        RABI_420_FREQ = get_rough_resonance(trapdepth=trapdepth)
        AVALANCHE_420_FREQ = RABI_420_FREQ-1 # MHz
        experiment.RABI_420_FREQ = RABI_420_FREQ

        for exp_idx, _ in enumerate(lifetime_repetitions_arr):
            # if exp_idx in np.arange(6): continue
            for _ in range(2):
                if exp_idx in [1,]: break
                trial_num = 0
                while True:
                    try:
            ...
            while True:
                ...
                try:
                    aborted = experiment.AOD_alignment(exp_idx=exp_idx, exp_name_prefix=EXP_NAME_PREFIX, exp_name_postfix=exp_postfix,
                                                timeout_control={"use": True, "timeout": 600})
                    if aborted:
                        # exp.hardware_controller.restart_zynq_control()
                        sleep(5)
                    break
            ...
            while True:
                if exp_idx in [0,]: break
                try:
                    aborted = experiment.lifetime(trap_depth=trapdepth,exp_idx=exp_idx, exp_name_prefix=EXP_NAME_PREFIX, exp_name_postfix=exp_postfix,
                                                timeout_control={"use": True, "timeout": 6500})
                    if aborted:
                        # exp.hardware_controller.restart_zynq_control()
                        sleep(5)
                    break
            ...
            while True:
                # if exp_idx==0: break
                try:
                    aborted = experiment.resonance(trap_depth=trapdepth, exp_idx=exp_idx, exp_name_prefix=EXP_NAME_PREFIX, exp_name_postfix=exp_postfix,
                                            timeout_control={"use": True, "timeout": 1200})
                    if aborted:
                        # exp.hardware_controller.restart_zynq_control()
                        sleep(5)
                    break
            ...
            while True:
                try:
                    aborted = experiment.rabi(trap_depth=trapdepth, exp_idx=exp_idx, exp_name_prefix=EXP_NAME_PREFIX, exp_name_postfix=exp_postfix,
                                            timeout_control={"use": True, "timeout": 1200})
                    if aborted:
                        # exp.hardware_controller.restart_zynq_control()
                        sleep(5)
