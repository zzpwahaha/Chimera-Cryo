...
config_name = "tweezerloading_RydbergRabi_wRearrangement.Config"
...
analysis_locs = da.DataAnalysis(year='2026', month='March', day='30', data_name='data_12', n_cluster_row=5,
                                window=window, thresholds=thresholds, binnings=binnings, 
                                multi_points_option = dict({"active":True, "search_square":4, "num_points":6}))


class ResonanceWithEOM:
...
00000000000000000000"""
    ...
    def resonance(self, exp_idx, amplitude, frequency, exp_name_prefix, exp_name_postfix = '', timeout_control={"use": True, "timeout": 1800}):
        ...
        self.move_EOM_resonance(end_freq=self.RABI_420_FREQ-3, step=0.25, channel=0)
        config_file.modify_parameter("STATIC_DDS", "Control?", 1)
        config_file.modify_parameter("STATIC_DDS", " DDS-0 Value:", f'{self.RABI_420_FREQ:.3f}-3+ryd420_eom_resonance')
        ...
        config_file.config_param.update_variable("ryd420_amplitude", constant_value=0.02) # -0.0072 #-0.0096
        config_file.config_param.update_variable("ryd1013_amplitude", constant_value=4)
        config_file.config_param.update_variable("time_scan_us", constant_value=0.21)
        ...
        self.CURRENT_EOM_FREQ = self.RABI_420_FREQ-3+6
        self.move_EOM_resonance(end_freq=self.RABI_420_FREQ, step=0.25, channel=0)
        data_analysis = da.DataAnalysis(YEAR, MONTH, DAY, exp_name, maximaLocs=analysis_locs.maximaLocs,
...
if __name__ == "__main__":
    ...
    EXP_NAME_PREFIX = "N-75-RESONANCE-RT"

    # MW_FREQUENCY = 7850 #6375 #7850 #6375.
    RABI_420_FREQ = 547.3 
    CURRENT_EOM_FREQ = 547.3-3  #RABI_420_FREQ

    experiment = ResonanceWithEOM(
        CURRENT_EOM_FREQ = CURRENT_EOM_FREQ,
        RABI_420_FREQ = RABI_420_FREQ
    )
    amplitudes = [15,12,9,6,3,0,-3,-6]
    frequencies = [6375, 7850]

    for exp_idx, amp in enumerate(amplitudes):

        for freq in frequencies:        
            eid = 0
            exp_postfix = ''
            while True:
                # if (exp_idx%3 != 0) or (exp_idx==0): break
                if True: break
                try:
            ...
            while True:
                try:
                    aborted = experiment.resonance(exp_idx=exp_idx, amplitude = amp, frequency=freq,
                                                exp_name_prefix=EXP_NAME_PREFIX + f'-MW-{amp:.2f}dBm-FREQ-{freq:d}MHz', exp_name_postfix=exp_postfix,
                                            timeout_control={"use": True, "timeout": 1800})
                    if aborted:
                        # exp.hardware_controller.restart_zynq_control()
                        sleep(5)
